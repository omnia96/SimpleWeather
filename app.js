...
App({
  onLaunch: function () {
    ...
    var that = this
    that.setGlobalCache()
    that.autoChangeTheme()
    
  },
  onShow: function () {},
  //自动切换夜间模式
  autoChangeTheme:function(){
    var date = new Date()
    var that = this
    var hour = date.getHours()
    // var hour = 20
    if(hour > 6 && hour < 20){
      that.globalData.themes = "themes_light"
    }else{
      that.globalData.themes = "themes_dark"
    }
  },
  ...
  setGlobalCache: function () {
    var that = this
    if (!that.getCache("userGlobalCache")) {
      that.setCache("userGlobalCache", "cache_ok")
      that.setCache("userCityList", [])
      that.setCache("weatherData",[])
    }
  },
